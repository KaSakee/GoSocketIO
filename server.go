...
import (
	...
	"github.com/KaSakee/GoSocketIO/protocol"
	"github.com/KaSakee/GoSocketIO/transport"
)
...
func (s *Server) SetupEventLoop(conn transport.Connection, remoteAddr string,
	...
	ID := generateNewId(remoteAddr)

	if len(r.Header.Get("_id")) > 0 {
		ID = r.Header.Get("_id")
	}

	println("connected_ID: " + ID)
